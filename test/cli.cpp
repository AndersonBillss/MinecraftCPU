...
TEST_CASE("CLI flags")
{
      ...
      parsed = parseHelper({"mcScript", "-s", "-v"}, [](Cli::Options &o)
                           { o.boolOption("sampleFlag", "s"),
                                 o.boolOption("verbose", "v"); });
      REQUIRE_THROWS_AS(parsed.ensureExclusive({"sampleFlag", "verbose"}), CliError);

      parsed = parseHelper({"mcScript", "-s", "-f"}, [](Cli::Options &o)
                           { o.boolOption("sampleFlag", "s");
                              o.boolOption("verbose", "v"); 
                              o.stringOption("file", "f")
                                    .addImplicit("Hello"); });
      REQUIRE_NOTHROW(parsed.ensureExclusive({"sampleFlag", "verbose"}));
