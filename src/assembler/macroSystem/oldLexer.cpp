...
std::set<char> OldAsmMacroLexer::operatorTokens = {
    '+',
    '-',
    '*',
    '/',
...
int handleOperator(std::vector<std::string> &tokens, const std::string &text, size_t index)
{
    ...
    while (index < text.size())
    {
        if (
            ...
            (index == text.size() - 1) ||
            (OldAsmMacroLexer::operatorTokens.find(text[index]) == OldAsmMacroLexer::operatorTokens.end()))
        {
...
std::vector<std::string> OldAsmMacroLexer::tokenize(const std::string &block, size_t startingIndex, size_t endingIndex)
{
